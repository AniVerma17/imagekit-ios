...
public struct MimeType  : Sendable {
  ...
  ///  - parameter mimeDetector: MimeDetector instance
  ///
  ///  - returns: Bool
  public func matches(bytes: [UInt8], mimeDetector: MimeDetector) -> Bool {
    return bytes.count >= bytesCount && matches(bytes, mimeDetector)
  }
  ...
  public static let all: [MimeType] = [
    ...
    MimeType(
      ...
      type: .mkv,
      bytesCount: 4,
      matches: { bytes, mimeDetector in
        guard bytes[0...3] == [0x1A, 0x45, 0xDF, 0xA3] else {
          return false
        }

        let _bytes = Array(mimeDetector.readBytes(count: 4100)[4 ..< 4100])
        var idPos = -1
